...
function isAbbreviation(token: string) {
  ...
  const formattedToken = token.replace(/['’]s$/i, '').replace(/\.+$/, '');
  return ABBREVIATIONS.has(formattedToken.toLowerCase());
}
...
export class TextSplitterStream {
  ...
  private process() {
    ...
    while (i < len) {
      ...
      if (stack.length === 0 && isSentenceTerminator(c)) {
        ...
        if (
          ...
          token.at(-1) &&
          !isSentenceTerminator(token.at(-1) as string)
        ) {
    ...
    this.buffer = buffer.substring(sentenceStart);
  ...
  async *[Symbol.asyncIterator](): AsyncGenerator<string, void, void> {
    ...
    while (true) {
      if (this.sentences.length > 0) {
        yield this.sentences.shift() as string;
      } else if (this.closed) {
